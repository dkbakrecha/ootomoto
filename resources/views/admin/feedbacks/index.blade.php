...
<div class="user-buttons">
    ...
    <a href="{{ route('admin.feedback.create') }}" type="button" class="btn btn-feedback">
...
<div style="margin-top:40px;">
    <div class="row" id="feedback-board">
        ...
        <div class="col-xs-4 col-sm-5 col-md-4 feedback-left {{ $class }}"> 
            <form action="{{ route('admin.feedbacks') }}" method="GET" role="search">
            ...
            @php $_user_id = $feedback->receiver->id @endphp
            @php $_user_type = $feedback->receiver->user_type @endphp
            @else
            @php $_user_id = $feedback->sender->id @endphp
            @php $_user_type = $feedback->sender->user_type @endphp
            @endif
            
            @php $_class = "" @endphp
            @if($_user_type == 0)
            @php $_class = "tile-shop" @endphp
            @elseif($_user_type == 1)
            @php $_class = "tile-supervisor" @endphp
            @elseif($_user_type == 2)
            @php $_class = "tile-customer" @endphp
            @endif
            <div class="left-feedbar">
                <div class="row feedback-tile {{ $_class }}" data-id="{{ $feedback->parent_id }}" data-user_id="{{ (!empty($feedback->receiver) && ($feedback->receiver->id != Auth::user()->id))?$feedback->receiver->id:$feedback->sender->id }}">
                    @if(!empty($feedback->receiver) && ($feedback->receiver->id != Auth::user()->id))
        ...
        <div class="col-xs-8 col-sm-7 col-md-8 feedback-right">
            ...
            <div class="reply-right-section">
                <form action="{{ route('admin.feedbackReply') }}" method="post" class="form-horizontal feedbackReplyForm" id="feedbackReplyForm">
...
<script type="text/javascript">
    $(document).ready(function () {
        $(".feedback-tile").click(function () {
            ...
            $(".feedback-tile").removeClass("active");
